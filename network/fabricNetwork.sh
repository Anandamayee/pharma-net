...
function replacePrivateKey() {
  ...
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR" || exit
  sed $OPTS "s/TRANSPOTER_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yml
...
function generateChannelArtifacts() {
  ...
  set -x
  configtxgen -profile OrdererGenesis -channelID pharma-sly-channel -outputBlock ./channel-artifacts/genesis.block
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for reatiler..."
    exit 1
  fi
  ...
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for transpirter..."
    exit 1
  fi
